...
#            x_validate_tdm = vectorizer.transform(x_validate)
            ...
            x_test_array = x_test_tdm.toarray()
...
#                x_validate_tdm_array.shape
            if a_dataset == 'test':
                y = y_test # for testset labels
                X = x_test_array #testset trans
                df_X_ValǀTest = pd.DataFrame(x_test_tdm_array,columns=vectorizer.get_feature_names()) #test df
            ...
            print('this resulted in a ratio of: ', RATIO)
...
#            for ALPHA in range(1,31):
            ...
            # for If you want to switch to predicting training/test/validation; for getting all 3 results
            model_toPredict = 'test'
            if model_toPredict == "training":
                y_true = y_train
            elif model_toPredict == 'test':
                y_true = y
                
            ŷ_nb = nb_classifier.predict(x_train_tdm_array)
            
            cR_nbBase = classification_report(y_true,ŷ_nb, zero_division=0)
            print('\n',cR_nbBase)
            
            #for i,j in zip(y_true, ŷ_nb):    print(i==j)
            print('\n format of CM:\n', np.array([    ['TN', 'FP'],
                                                      ['FN', 'TP']]) )
            print('\n*****', nb_classifier, '*****\n\nBASE NB')
            #print(str( vectorizer)[:13])
            print(confusion_matrix(y_true,ŷ_nb))
            
            TNψfn_nb_base = confusion_matrix(y_true,ŷ_nb)[0,0] + confusion_matrix(y_true,ŷ_nb)[1,1]
            print('tn+tp',TNψfn_nb_base)
            
            F1_scoreBaseline_mnb = round(f1_score(y_true,ŷ_nb,average='macro'),3)
...
#            for C in range(4,5):
#                    print(c/10)
            svm_classifier = svm.SVC(C=0.5,kernel='linear', degree=3,gamma='auto',probability=True)
