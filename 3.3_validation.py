...
#    SEED = 2 # used to be 7 # Note does not affact Test set, as there is nothing to split
    ...
    tNψtp.append(('seed:',SEED,'mnbBase:',TNψfn_nb_base, 'mnbEnriched:',TNψfn_nb_enriched))
    ...
    tNψtp.append(('seed:',SEED,'svmBase:',TNψfn_svm_base,'svmEnriched:',TNψfn_svm_enriched))
...
#    print('\n\n tNψtp:',tNψtp)
    ...
    #    from my_FunctionsModule_ForThesis import paraPickleSaverENRICHED
        allParamaters = {'.raretermHyper':RARETERM,  'vectparas': vectorizer.get_params(), 'neighborsLEN':len(NEIGHBORS), 'neighborsLen_set':len(set(NEIGHBORS)), '.SIMILARITY_HYPERPARA':SIMILARITY_HYPERPARA,'clfParams': CLF.get_params()
     }
        paraPickleSaverBASE(CLF)
        print(str(CLF)[:3])
        paraPickleSaverENRICHED(CLF)
        print(str(CLF)[:3])
        print('rareterm',RARETERM)
        print('****************************************************')
