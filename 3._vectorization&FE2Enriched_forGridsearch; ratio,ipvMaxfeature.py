...
RatioHyper = 0.4# determines the ratio of rareterms/maxfeatures: i.e. 0.3 = 30% of the max features is the absolute rare term integer: eg 100 max features * 0.3 = rareterm = 30; this is because the ratio determines the enrichement %; now it is robust, if max feature changes, there is no problem
...
# LEMMA= 
MAXFEATURES = [20000]#,12000,28000,5000,10000,15000,25000,30000,35000]#,16000,21000,18000,19000,17000,25000,8000]#, 15000,20000,25000,30000,32324]# 4000, 5000, 6000]
MAX_DF_LIST = [1.0]
MIN_DF = 1
...
#               for I in range(1,10): # if you want to hyper tuning
                nb_classifier = MultinomialNB(alpha=1)
                nb_classifier.fit(x_train, y_train)
                
                ŷ_enriched_nb = nb_classifier.predict(df_aggregated) #  the predicted class
            
                print ('\n with the para:', nb_classifier)
                
                ŷ_enriched_nb = nb_classifier.predict(df_aggregated) # IT WAS : ŷ_enriched_nb = nb_classifier.predict(pd.DataFrame(x_test))<<< ALL THAT TIME
                           
                
                #for i,j in zip(y_test, ŷ_enriched_nb):    print(i==j)
                print('\n format of CM:\n', np.array([    ['TN', 'FP'],
                                                          ['FN', 'TP']]) )
                print('\n***', nb_classifier, '***\n\nENRICHED NB')
                #print(str( vectorizer)[:13])
                print(confusion_matrix(y_test,ŷ_enriched_nb))
                TNψfn_nb_enriched = confusion_matrix(y_test,ŷ_enriched_nb)[0,0] + confusion_matrix(y_test,ŷ_enriched_nb)[1,1]
                print('tn+tp',TNψfn_nb_enriched)
                
                print(classification_report(y_test,ŷ_enriched_nb, zero_division=0))
                #print('accuracy_score',accuracy_score(y_test, ŷ_enriched_nb))
                F1_score_enriched_mnb = round(f1_score(y_test,ŷ_enriched_nb,average='macro'),3)
                print('\n\n***f1_score_macro enriched NB:',f1_score(y_test,ŷ_enriched_nb,average='macro'),'***\n \n')
                ...
                # =============================================================================
    #                    from sklearn import svm
                svm_classifier = svm.SVC(C=1,kernel='linear', degree=3,gamma='auto',probability=True)
                ...
                def paraPickleSaverBASE(clf):
                    ...
                    with open(PATH+f'{TIME};_Base_____f1={F1_score};_TnTp={TnTp};_CLF={CLF_name};__{VECTORIZER_TYPE};_max_features={MAX_FEATURES};_ngram={NGRAM_RANGE};_max_df={MAX_DF};_min_df={MIN_DF};__similarity={SIMILARITY_HYPERPARA};_rareterm={RARETERM};_ratioRareMax={RatioHyper};_Seed={SEED}_.thesis', 'wb') as f:
                ...
                def paraPickleSaverENRICHED(clf):
                    ...
                    PATH = 'C:\\Users\Sa\\WD_thesisPython_workdrive\\Text_Classification_Pipeline\\'
                    # here I could add in {} WHAT I want to SHOW UP IN FILENAME.. COEFS AND METRICS.. ?WRITE TO EXCEL FILE?
                    with open(PATH+f'{TIME};_Enriched_f1={F1_score};_TnTp={TnTp};_CLF={CLF_name};__{VECTORIZER_TYPE};_max_features={MAX_FEATURES};_ngram={NGRAM_RANGE};_max_df={MAX_DF};_min_df={MIN_DF};__similar={SIMILARITY_HYPERPARA};_rareterm={RARETERM};_ratioRareMax={RatioHyper};_enriched={Enriched_percentage};__topn={TOP_NEIGHBORS};_d={DIMENSION};_Seed={SEED};_{WORDVEC_TYPE}_.thesis', 'wb') as f:
                        pickle.dump([F1_score ,allParamaters],f)
